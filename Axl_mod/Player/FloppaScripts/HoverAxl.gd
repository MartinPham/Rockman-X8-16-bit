...
func _Setup():
	...
	if not upgraded:
		max_jump_time = 2.5
		#print("Normal Axl hover - extended duration: ", max_jump_time, " seconds")
	else:
		max_jump_time = 999.0
		#print("Upgraded Axl hover - unlimited duration")
...
func should_hover() -> bool:
	...
	if hover_locked:
		if not character.is_on_floor() and not facing_a_wall():
			if upgraded or not Has_time_ran_out():
				return true
