...
func _ready() -> void :
	...
	# Create diagonal dash counter UI
	call_deferred("create_diagonal_dash_counter")
...
func equip_axl_parts() -> void :
	...
	dash.dash_duration = 0.55  # Match Zero
	dash.horizontal_velocity = 260  # Match Zero
	var airdash = get_node("AirDash")
	airdash.upgraded = false
	airdash.dash_duration = 0.55  # Match Zero
	airdash.horizontal_velocity = 260  # Match Zero
	...
	get_node("DashWallJump").horizontal_velocity = 260  # Match Zero dash speed
	get_node("DashJump").horizontal_velocity = 260  # Match Zero dash speed
	get_node("Fall").dash_momentum = 260  # Match Zero dash speed
	var _afterimagen = animatedSprite.get_node("afterImagesn")
	_afterimagen.upgraded = false
...
func equip_axl_white_parts() -> void :
	...
	var dash = get_node("Dash")
	dash.dash_duration = 0.55  # Match Black Zero
	dash.horizontal_velocity = 300  # Match Black Zero
	var airdash = get_node("AirDash")
	airdash.upgraded = true
	airdash.dash_duration = 0.55  # Match Black Zero
	airdash.horizontal_velocity = 300  # Match Black Zero
	...
	get_node("DashWallJump").horizontal_velocity = 300  # Match Black Zero dash speed
	get_node("DashJump").horizontal_velocity = 300  # Match Black Zero dash speed
	get_node("Fall").dash_momentum = 300  # Match Black Zero dash speed
	var _afterimagen = animatedSprite.get_node("afterImagesn")
	_afterimagen.upgraded = false
...
func create_diagonal_dash_counter() -> void:
	# Create a separate CanvasLayer with high layer number for the counter
	yield(get_tree(), "idle_frame")
	
	# Create a new CanvasLayer specifically for the counter
	var counter_layer = CanvasLayer.new()
	counter_layer.name = "DiagonalDashCounterLayer"
	counter_layer.layer = 1  # Same layer as HUD, so pause menu covers it
	
	# Create a Control node that will draw the text
	var counter_display = Control.new()
	counter_display.name = "DiagonalDashCounter"
	counter_display.mouse_filter = Control.MOUSE_FILTER_IGNORE
	
	# Position on the LEFT MIDDLE side
	counter_display.anchor_left = 0.0   # Left side
	counter_display.anchor_top = 0.5    # Middle vertically
	counter_display.anchor_right = 0.0
	counter_display.anchor_bottom = 0.5
	counter_display.margin_left = 10     # 10 pixels from left edge
	counter_display.margin_top = -20     # Center it vertically
	counter_display.margin_right = 100   # Width of 90 pixels
	counter_display.margin_bottom = 20   # Height of 40 pixels
	counter_display.rect_min_size = Vector2(90, 40)	# Load font
	var font = load("res://src/Fonts/Xclassicfont.fnt")
	
	# Attach the counter script
	var counter_script = load("res://Axl_mod/Player/FloppaScripts/DiagonalDashCounter.gd")
	if counter_script:
		counter_display.set_script(counter_script)
		# Pass the font to the script
		if font:
			counter_display.set("display_font", font)
	
	# Add to CanvasLayer
	counter_layer.add_child(counter_display)
	
	# Add the CanvasLayer to the scene root
	get_tree().current_scene.add_child(counter_layer)
