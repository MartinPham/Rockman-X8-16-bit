...
const user_data: String = "user://username"
func save_user(_username: String) -> void :
	var save_data = {
		"username": _username, 
		"disclaimer_accepted": DISCLAIMER_ACCPETED, 
	}
	var json = Encryption.canonical_json_stringify(save_data)
	var encrypted_dict = Encryption.encrypt_sha256_aes_cbc(json, "username", "data")
	var encrypted_json = Encryption.canonical_json_stringify(encrypted_dict)
	
	var file = File.new()
	if file.open(user_data, File.WRITE) == OK:
...
func load_user() -> void :
	...
	if file.file_exists(user_data):
		...
		var save_data = Encryption.decrypt_sha256_aes_cbc(enc_data["username"], enc_data["data"])

		if typeof(save_data) == TYPE_DICTIONARY:
			USERNAME = save_data.get("username", "")
			DISCLAIMER_ACCPETED = save_data.get("disclaimer_accepted", false)
	else:
		USERNAME = ""
		DISCLAIMER_ACCPETED = false
...
func _save() -> void :
	...
	var json = Encryption.canonical_json_stringify(save_data)
	var encrypted_dict = Encryption.encrypt_sha256_aes_cbc(json, "char", "data")
	var encrypted_json = Encryption.canonical_json_stringify(encrypted_dict)
...
func _load() -> void :
	...
	if file.file_exists(char_data):
		...
		var enc_data = encrypted_dict.result
		var save_data = Encryption.decrypt_sha256_aes_cbc(enc_data["char"], enc_data["data"])
...
func set_player_character(player_character) -> void :
...
func get_player_character_string() -> String:
	return CharacterManager.player_character
