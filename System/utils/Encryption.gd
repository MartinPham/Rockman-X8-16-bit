...
func canonical_json_stringify(value) -> String:
...
func encrypt_sha256_aes_cbc(_data: String, _iv: String, _payload: String) -> Dictionary:
...
func decrypt_sha256_aes_cbc(iv_b64: String, payload_b64: String) -> Dictionary:
	var key: PoolByteArray = hex_to_bytes(get_secret_key())
...
func add_pkcs7_padding(data: PoolByteArray, block_size: int) -> PoolByteArray:
	var pad_len: int = block_size - (data.size() %block_size)
	for i in range(pad_len):
		data.append(pad_len)
	return data

func remove_pkcs7_padding(data: PoolByteArray) -> PoolByteArray:
	if data.size() == 0:
		return data
...
func hex_to_bytes(hex_str: String) -> PoolByteArray:
...
func parse_hex_byte(hex_byte: String) -> int:
...
func secure_wipe_bytes(arr: PoolByteArray) -> void :
	arr.fill(0)
	arr.resize(0)
