...
func _ready() -> void :
	_on_focus_exited()
	icon.material.set_shader_param("grayscale", not CharacterManager.awakened_zero_armor)
...
func toggle_custom_zero() -> void :
	CharacterManager.awakened_zero_armor = not CharacterManager.awakened_zero_armor
	if CharacterManager.awakened_zero_armor:
		Tools.timer(0.075, "play", equip)
		char_name.text = "Zero (Awakened)"
	else:
