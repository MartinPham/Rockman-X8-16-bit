...
export  var collectible_name: String = "awakened_zero_armor"
export  var dialogue_zero: Resource
onready var capsule_sound: AudioStreamPlayer2D = $capsule
...
func _ready() -> void :
	...
	Event.listen("dialog_concluded", self, "vanish")
	Event.listen("capsule_open", self, "start")
	timer = 0.0
	timer2 = 0.0
	charge_state = 0
	finished = false
	capsule_sound.stream.loop = true
	call_deferred("set_dialogue")
	#call_deferred("handle_player_not_zero")
...
func handle_player_not_zero() -> void :
	if not "Zero" in CharacterManager.team:
		finished = true
...
func _on_area2D_body_entered(body: Node) -> void :
	if CharacterManager.player_character == "Zero" and not CharacterManager.betazero_activated:
...
func _process(delta: float) -> void :
	if not finished:
		...
		if charge_state == 1:
			...
			GameManager.player.play_animation_once("awaken_land")
			if timer > 1.2:
				GameManager.player.play_animation_once("awaken_equip")
				if CharacterManager.player_character == "Zero":
				...
				CharacterManager.set_zeroX8_colors($awakeneffect)
				$boom.play()
