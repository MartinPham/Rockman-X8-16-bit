...
export  var collectible_name: String = "floppa_axl_activated"
export  var dialogue_axl: Resource
...
func _ready() -> void :
	...
	#call_deferred("handle_player_not_axl")
	
func set_dialogue() -> void :
	if CharacterManager.player_character == "Axl":
		dialogue = dialogue_axl
	else:
		dialogue = dialogue_axl

func handle_player_not_axl() -> void :
	if not "Axl" in CharacterManager.team:
		finished = true
