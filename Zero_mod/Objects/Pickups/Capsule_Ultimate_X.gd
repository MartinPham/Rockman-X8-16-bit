...
export  var collectible_name1: String = "ultima_head"
export  var collectible_name2: String = "ultima_body"
export  var collectible_name3: String = "ultima_arms"
export  var collectible_name4: String = "ultima_legs"
export  var dialogue_x: Resource
export  var dialogue_not_x: Resource
export  var dialogue_axl_special: Resource
...
func handle_already_got_armor() -> void :
	if collectible_name1 in GameManager.collectibles:
		finished = true
		call_deferred("disable_glass_collider")
		sprite.play("finished")
		if debug_logs:
			print_debug("Capsule: Player already has " + collectible_name1)
...
func _process(delta: float) -> void :
	if not finished:
		...
		if charge_state == 1:
			...
			if timer > 0.4:
				if not gave_armor:
					Event.emit_signal("collected", collectible_name1)
					Event.emit_signal("collected", collectible_name2)
					Event.emit_signal("collected", collectible_name3)
					Event.emit_signal("collected", collectible_name4)
