...
onready var _saber_sprites = preload("res://Zero_mod/Sprites/zero.res")
...
func change_ride_chaser_sprites():
	...
	var _reference_frames = load("res://Zero_mod/Sprites/zero.res")
...
func equip_zero_parts():
	...
	for saber_node in saber_nodes:
		saber_node.hitbox_extra_damage = dmg_multiplikator
		saber_node.hitbox_extra_damage_boss = dmg_multiplikator
		saber_node.hitbox_extra_damage_weakness = dmg_multiplikator
		saber_node.hitbox_extra_break_guard_value = dmg_multiplikator

	get_node("Youdantotsu").hitbox_upgraded = true
	for saber_node in saber_nodes:
		if saber_node != get_node("Youdantotsu"):
			saber_node.hitbox_upgraded = false
