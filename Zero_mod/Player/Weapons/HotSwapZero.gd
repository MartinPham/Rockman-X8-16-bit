...
const travel_distance: float = 400.0
...
onready var cursor_see: Sprite = $cursor_see
...
func _input(event: InputEvent) -> void :
	...
	for input in analogs:
		if event.is_action_pressed(input):
			if event is InputEventKey or event is InputEventJoypadButton:
				...
				cursor_see.modulate.a = 0.0
				using_buttons = true
				break
			elif event is InputEventJoypadMotion:
				cursor.modulate.a = 0.0
				cursor_see.modulate.a = 0.2
				using_buttons = false
				break
	...
	if using_buttons:
		if event.is_action_pressed("analog_left"):
			...
			cursor_see.position.x = - travel_distance * 0.1
			tweenx.reset()
		...
		if event.is_action_pressed("analog_up"):
			...
			cursor_see.position.y = - travel_distance * 0.1
			tweeny.reset()
...
func slowly_reset_x_cursor():
	...
	tweenx.attribute("position:x", 0.0, 0.1, cursor_see)
	
func slowly_reset_y_cursor():
	tweeny.reset()
	tweeny.attribute("position:y", 0.0, 0.1, cursor)
	tweeny.attribute("position:y", 0.0, 0.1, cursor_see)
...
func _physics_process(delta: float) -> void :
	if not using_buttons:
		...
		cursor_see.position.x = (analog_right - analog_left) * travel_distance
		cursor_see.position.y = (analog_down - analog_up) * travel_distance
...
func _on_cursor_area_entered(area: Area2D) -> void :
	...
	if selected.selectable and not selected.already_selected:
		...
		Event.emit_signal("select_weapon_zero")
