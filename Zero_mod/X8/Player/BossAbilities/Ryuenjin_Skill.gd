...
var loop_frame: int = 11
var loop_start_frame: int = 8
var jump_start_frame: int = 4
...
func hitbox_and_position() -> void :
	...
	if animatedSprite.frame >= jump_start_frame and animatedSprite.frame < loop_frame:
		if character.saber_node.current_weapon.name == "Saber":
			...
			if animatedSprite.frame >= 4 and animatedSprite.frame < 5 or animatedSprite.frame >= 8 and animatedSprite.frame < 11:
				...
				hitbox_downright = Vector2(65, 30)
			else:
				hitbox_upleft = Vector2(0, - 58)
				hitbox_downright = Vector2(65, 30)
...
func movement_speed_frames() -> bool:
	return animatedSprite.frame >= jump_start_frame and animatedSprite.frame < loop_frame

func jump_frame() -> void :
	if not jumped:
		if animatedSprite.frame >= jump_start_frame:
...
func set_saber_animations() -> void :
	animatedSprite.animation = "ryuenjin"
	animatedSprite.frame = 0
	saber_sound.play()
	if CharacterManager.player_character == "Zero":
		if CharacterManager.awakened_zero_armor:
			create_swordwave(global_position, 20, 600)
		elif CharacterManager.black_zero_armor and not CharacterManager.awakened_zero_armor:
			create_swordwave(global_position, 12, 420)
		else:
			create_swordwave(global_position, 10, 420)


func create_swordwave(ground_position,  damage_value, speed) -> void :
	var facing_direction = get_facing_direction()
	...
	instance.scale.x = facing_direction
	instance.scale.y = facing_direction
	instance.set_horizontal_speed(speed * facing_direction)
...
func _StartCondition() -> bool:
	if not CharacterManager.extra_saber_combo:
		return false
	if not CharacterManager.extra_saber_03:
		return false
	...
	if _animation in character.saber_animations:
		if _animation == "saber_6" and animatedSprite.frame >= 6:
			return true
		if _animation == "saber_dash" and animatedSprite.frame >= 5:
			return true
		if _animation == "saber_dash2":
			return false
		return false
	if _animation == "dash":
		return true
