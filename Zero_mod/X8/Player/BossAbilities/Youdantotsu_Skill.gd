...
onready var blastlaunch: PackedScene = preload("res://src/Actors/Player/BossWeapons/BlastLauncher/BlastLaunchCharged.tscn")
var creator: Node2D
...
func hitbox_and_position() -> void :
	if animatedSprite.animation == "youdantotsuawakena" or animatedSprite.animation == "youdantotsuawaken":
		hitbox_damage = 8
		...
		if animatedSprite.frame >= 5 and animatedSprite.frame < 12:
			hitbox_upleft = Vector2(0, - 50)
			hitbox_downright = Vector2(175, 45)
			spawn_hitbox(hitbox_upleft, hitbox_downright)

	if animatedSprite.animation == "youdantotsu" or animatedSprite.animation == "youdantotsua":
...
func set_saber_animations() -> void :
	...
	if CharacterManager.player_character == "Zero":
		if CharacterManager.custom_zero_armor:
			if CharacterManager.awakened_zero_armor and character.saber_node.current_weapon.name == "Saber":
				animatedSprite.animation = "youdantotsuawakena"
			else:
				animatedSprite.animation = "youdantotsua"
		else:
			if CharacterManager.awakened_zero_armor and character.saber_node.current_weapon.name == "Saber":
				animatedSprite.animation = "youdantotsuawaken"
			else:
				animatedSprite.animation = "youdantotsu"
...
func _EndCondition() -> bool:
	var _animation = get_parent().get_animation()
	if _animation in character.saber_animations:
		if not _animation == "youdantotsu" and not _animation == "youdantotsua" and not _animation == "youdantotsuawaken" and not _animation == "youdantotsuawakena":
			return true
		if end_saber_state():
			return true
	else:
...
func _Setup() -> void :
	if character.saber_node.current_weapon.name == "Saber":
		if CharacterManager.awakened_zero_armor:
			effect_emit("youdantotsu_awaken")
		else:
		...
		effect_emit("youdantotsu_hanger")

	if character.saber_node.current_weapon.name == "Z-Breaker":
		zbuster_sound.play()
		create_blastlaunch(global_position)

	elif character.saber_node.current_weapon.name == "Rogue-Blade":
		effect_emit("youdantotsu_rogue")
...
func create_blastlaunch(ground_position) -> void :
	var instance = blastlaunch.instance()
	ground_position.y += -9
