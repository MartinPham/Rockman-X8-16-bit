...
func hide_far_trails():
	if abs(global_position.x - origin.x) < 170 and abs(global_position.y - origin.y) < 170:
		hidden_trails = [trail_3, trail_4]
		
	elif abs(global_position.x - origin.x) < 280 and abs(global_position.y - origin.y) < 280:
