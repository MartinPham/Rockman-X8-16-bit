...
onready var awakenaura: AnimatedSprite = $"../animatedSprite/effect"

func connect_start_events() -> void:
	Event.listen("teleport_to_vileex",self,"prepare_for_intro")
	Event.listen("vileex_eject",self,"_on_vile_eject")
	Event.listen("vileex_defeated",self,"defeated")
	prepare_for_intro()

func defeated() -> void:
	awakenaura.visible = false
