...
var awakened_casted: bool = false
...
func set_game_modes() -> void :
	...
	if CharacterManager.game_mode == 2:
		flicker_repeats = 100
		flicker_pattern = [0.3, 0.15, 0.15]
		lighting_pattern = generate_lighting_pattern(Tween.EASE_IN_OUT, Tween.TRANS_SINE, [180, 225, 270, 315, 360], 30.0, 0.5, flicker_pattern)
	if CharacterManager.game_mode >= 3:
		flicker_repeats = 100
		flicker_pattern = [0.01, 0.15, 0.15]
		lighting_pattern = generate_lighting_pattern(Tween.EASE_IN_OUT, Tween.TRANS_SINE, [180, 225, 270, 315, 360], 30.0, 0.5, flicker_pattern)
	if awakened_casted:
		flicker_repeats = 100
		flicker_pattern = [0.3, 0.15, 0.15]
		lighting_pattern = generate_lighting_pattern(Tween.EASE_IN_OUT, Tween.TRANS_SINE, [1800, 2250, 2700, 3150, 3600], 30.0, 0.1, flicker_pattern)
