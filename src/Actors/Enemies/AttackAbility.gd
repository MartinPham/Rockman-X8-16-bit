...
export  var desperation_attack: bool = false

var decaying_speed: float = 100.0
...
func reset_decay(new_velocity: = 0.0) -> void :
	if new_velocity == 0:
		decaying_speed = get_horizontal_velocity()
	else:
		decaying_speed = new_velocity

func decay_horizontal_speed(time: float = 10.0, delta: float = 0.016):
...
func reduce_value_over_time(value: float, delta: float = 0.016, inverse_duration_of_decay: float = 10.0, threshold: float = 5) -> float:
	if value > threshold:
