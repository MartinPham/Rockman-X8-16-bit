...
onready var zero_fill: TextureProgress = $zero_fill
onready var zero_fill_texture: Texture = preload("res://Zero_mod/HUD/special_bar_fill.png")
...
func _process(delta: float) -> void :
	...
	if ultimate_weapon:
		if get_current_set() == "zero":
			ultimate_fill.texture_progress = zero_fill_texture
			#if CharacterManager.black_zero_armor:
				#u_blink.texture_progress = empty_bar
				#ultimate_fill.material = load("res://Zero_mod/X8/Sprites/ZeroX8_Material_Shader.tres")
				#CharacterManager.set_zeroX8_colors(ultimate_fill)
			...
			if is_instance_valid(GameManager.player):
				var _weapon = GameManager.player.get_node("Special").current_weapon
				if _weapon:
					blink_value = (_weapon.ammo_per_shot / ultimate_weapon.max_ammo) * 43
			var fill_value = get_bar_value(ultimate_weapon)
			ultimate_fill.value = fill_value
			if fill_value >= blink_value:
				u_blink.visible = true
			else:
				u_blink.visible = false
...
func _on_WeaponBar_displayed(weapon, char_name) -> void :
	if char_name == get_team_member().name:
		if weapon.name == "XDrive" or weapon.name == "GigaCrash" or weapon.name == "NovaStrike" or weapon.name == "Genmuzero":
			hide()
		else:
			unhide()
	else:
...
func check() -> void :
	...
	if get_current_set() == "hermes" or get_current_set() == "icarus" or get_current_set() == "ultimate" or get_current_set() == "axl" or get_current_set() == "zero":
