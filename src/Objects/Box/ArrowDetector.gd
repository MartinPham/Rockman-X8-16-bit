		...
		if "DarkArrowCharged" in body.name:
			...
			if body.upgraded:
				if body.break_guards:
					...
					on_saber_hit(5)
		elif "RogueSwordWave" in body.name:
			on_darkarrow_hit(1)
			body._OnHit(100)
		elif "RogueWave" in body.name:
			on_darkarrow_hit(3)
			body._OnHit(0)
		elif "SwordWave" in body.name:
			on_darkarrow_hit(1)
			body._OnHit(100)
		elif "ZBuster" in body.name:
			on_darkarrow_hit(1)
			body._OnHit(100)
		elif "HangerArrow" in body.name:
			on_darkarrow_hit(1)
			body._OnHit(100)
		elif "Rekohouha" in body.name:
			on_darkarrow_hit(3)
			body._OnHit(0)
		elif "Shingetsurin" in body.name:
			on_darkarrow_hit(3)
			body._OnHit(0)
		elif "Genmuzerowave" in body.name:
			on_darkarrow_hit(3)
			body._OnHit(0)
