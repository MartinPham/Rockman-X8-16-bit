...
var ta_status: String = "Recording..."

var ghost_file: String = "user://score.save"

var maximum_distance: Vector2 = Vector2(480, 320)
var maximum_bike_distance: Vector2 = Vector2(199, 100)
var debug_go_to_next_stage: bool = false
...
func _ready() -> void :
	...
	if args.size() > 0:
		if "--betabuild" in args:
			BETA = true


	if BETA:
		version = "1.0.0.4 hotfix 5"
		current_demo = " BETA"
	set_pause_mode(2)
...
var won_against_final_boss: bool = false
...
func is_on_camera(object: Node) -> bool:
	...
	var max_distance_from_camera_center: Vector2 = Vector2(196 + 64, 112 + 64)
...
func is_nearby(object1: Node, object2: Node, distance: Vector2) -> bool:
	if not is_instance_valid(object1):
		...
		if not object1.is_inside_tree() and not object2.is_inside_tree():
			return abs(object1.position.x - object2.position.x) < distance.x and abs(object1.position.y - object2.position.y) < distance.y
			
		elif not object1.is_inside_tree():
			return abs(object1.position.x - object2.global_position.x) < distance.x and abs(object1.position.y - object2.global_position.y) < distance.y
			
		elif not object2.is_inside_tree():
			return abs(object1.global_position.x - object2.global.x) < distance.x and abs(object1.global_position.y - object2.global.y) < distance.y
			
		return abs(object1.global_position.x - object2.global_position.x) < distance.x and abs(object1.global_position.y - object2.global_position.y) < distance.y

func is_pos_nearby(pos1: Vector2, pos2: Vector2, distance: Vector2) -> bool:
	return abs(pos1.x - pos2.x) < distance.x and abs(pos1.y - pos2.y) < distance.y

func save_stage_start_msec() -> void :
