...
func save_config_data() -> void :
	...
	var json = Encryption.canonical_json_stringify(config_data)
	var encrypted_dict = Encryption.encrypt_sha256_aes_cbc(json, "file", "config")
...
func write_to_file(slot: int = 0) -> void :
	...
	var file = File.new()
	var json = Encryption.canonical_json_stringify(game_data)
	var encrypted_dict = Encryption.encrypt_sha256_aes_cbc(json, "file", "save")
	var encrypted_json = Encryption.canonical_json_stringify(encrypted_dict)
...
func load_latest_save(max_slots: int = 10) -> void :
	var latest_time: = - 1
	var latest_slot: = - 1
...
func apply_data(slot: int = 0) -> void :
